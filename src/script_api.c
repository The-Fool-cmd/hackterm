...
static const char *core_result_to_string(CoreResult r) {
    switch (r) {
        ...
        case CORE_ERR_NOT_FOUND: return "NOT_FOUND";
        case CORE_ERR_NOT_LINKED: return "NOT_LINKED";
        case CORE_ERR_FILE: return "FILE_ERROR";
        case CORE_ERR_INVALID_ARG: return "INVALID_ARG";
...
static int l_game_get_current(lua_State *L) {
    ...
    lua_newtable(L);
    lua_pushstring(L, "id"); lua_pushinteger(L, s->id); lua_settable(L, -3);
    lua_pushstring(L, "name"); lua_pushstring(L, s->name); lua_settable(L, -3);
    lua_pushstring(L, "security"); lua_pushinteger(L, s->security); lua_settable(L, -3);
    lua_pushstring(L, "money"); lua_pushinteger(L, s->money); lua_settable(L, -3);
    return 1;
}
...
static int l_script_log(lua_State *L) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        off += copy;
        if (i < n && off + 1 < sizeof(tmp)) tmp[off++] = ' ';
        lua_pop(L, 1); /* remove tostring result */
        if (off + 1 >= sizeof(tmp)) break;
    }
    tmp[off] = '\0';
    script_log("%s", tmp);
    return 0;
